...
export const WildLife = () => {
	...
	const initialPage = parseInt(localStorage.getItem('currentPageF')) || 0;

	const [currentPageF, setCurrentPageF] = useState(initialPage);

	const updateItems = () => {
		let valor = store.category_pictures;
		setItems([...valor].splice(currentPageF * items_per_page, items_per_page))
		
	}
  
	let getinfo = async () => {
		
		await actions.geFilteredPicture("fauna");
		updateItems();
	...
	const nextPage = () => {
		...
		localStorage.setItem('currentPageF', nextPage);
	...
	return (
		<div className=" w-full">
			...
			<div className="pb-8 bg-black" >
				<p className="text-7xl text-center text-white pt-8 font-bold">Vida Salveje</p>
			...
			<div className="flex justify-around text-3xl text-white py-3 bg-black" >
				...
				>  Anterior </button>
				<p>Pag {currentPageF + 1} de {Math.ceil((store.category_pictures.length) / 12)}</p>
